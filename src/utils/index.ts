...
export const initConfig = async () => {
  ...
  try {
    ...
    if (resolvedProject) {
      ...
      console.log(`[Vertex Init] Set GOOGLE_CLOUD_PROJECT and VERTEX_GEMINI_PROJECT to: ${resolvedProject}`);
    }
    if (finalLocation) {
      process.env.VERTEX_GEMINI_LOCATION = finalLocation;
      console.log(`[Vertex Init] Set VERTEX_GEMINI_LOCATION to: ${finalLocation}`);
    }
    if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
      console.log(`[Vertex Init] Set GOOGLE_APPLICATION_CREDENTIALS to: ${process.env.GOOGLE_APPLICATION_CREDENTIALS}`);
    }
  } catch (e) {
